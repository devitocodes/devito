    ...
    def __init__(self, *args, **kwargs):
        ...
        for i in func:
            if isinstance(i, devito.Function):
                ...
                so = max(so, i.space_order)
            if isinstance(i, devito.TimeFunction):
                sp_dims += i.space_dimensions
                time_dims += i.time_dim
                so = max(so, i.space_order)
                is_time = True
                to = max(to, i.time_order)

        self.space_order = so
        self.time_order = to
        self.space_dimensions = tuple(set(sp_dims))
        if is_time:
            self.time_dim = tuple(set(time_dims))[0]
        self.is_TimeFunction = is_time
        devito.finite_difference.initialize_derivatives(self)
...
class Add(sympy.Add, FrozenExpr):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.space_dimensions = tuple(set(sp_dims))
        if is_time:
            self.time_dim = tuple(set(time_dims))
        self.is_TimeFunction = is_time
        devito.finite_difference.initialize_derivatives(self)
