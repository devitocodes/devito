...
class Operator(OperatorRunnable):
    ...
    def __getstate__(self):
        ...
        if isinstance(self.yk_soln, YaskNullKernel):
            return state
        state['yk_soln'] = None
        state['yk-soln-name'] = self.yk_soln.name
        state['yk-soln-soname'] = self.yk_soln.soname
        libfile = Path(namespace['yask-lib'], 'lib%s.so' % self.yk_soln.soname)
        with open(libfile, 'rb') as f:
            state['libfile'] = f.read()
        pyfile = Path(namespace['yask-pylib'], '%s.py' % self.yk_soln.name)
        with open(pyfile, 'r') as f:
            state['pyfile'] = f.read().encode()
        pysofile = Path(namespace['yask-pylib'], '_%s.so' % self.yk_soln.name)
        with open(pysofile, 'rb') as f:
            state['pysofile'] = f.read()
        return state

    def __setstate__(self, state):
        upk_libfile = state.pop('libfile', None)
        upk_pyfile = state.pop('pyfile', None)
        upk_pysofile = state.pop('pysofile', None)
        super(Operator, self).__setstate__(state)
        if isinstance(state['yk_soln'], YaskNullKernel):
            return

        libfile = Path(namespace['yask-lib'], 'lib%s.so' % state['yk-soln-soname'])
        if not libfile.is_file():
            with open(libfile, 'wb') as f:
                f.write(upk_libfile)
        pyfile = Path(namespace['yask-pylib'], '%s.py' % state['yk-soln-name'])
        if not pyfile.is_file():
            with open(pyfile, 'w') as f:
                f.write(upk_pyfile.decode())
        pysofile = Path(namespace['yask-pylib'], '_%s.so' % state['yk-soln-name'])
        if not pysofile.is_file():
            with open(pysofile, 'wb') as f:
                f.write(upk_pysofile)

        # Finally reconstruct the YASK solution -- no code generation or jit
        # will happen at this point, as all necessary files have been restored
        local_grids = [i for i in self.parameters if i.is_Array]
        context = contexts.fetch(self.input[0].grid, self._dtype)  # FIXME
        self.yk_soln = context.make_yk_solution(state['yk-soln-name'], None, local_grids)
