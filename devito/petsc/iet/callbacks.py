...
class BaseCallbackBuilder:
    ...
    def _make_core(self):
        ...
        self._make_formrhs()
        self._make_optional_callbacks()
        self._make_user_struct_efunc()

    def _make_optional_callbacks(self):
        """
        Hook for mixins to add optional callbacks.
        """
        pass
...
class InitialGuessCallbackMixin:
    """
    Mixin that adds the `FormInitialGuess` callback.
    """
    def _make_optional_callbacks(self):
    ...
    def _make_initial_guess(self):
        ...
        body = self._create_initial_guess_body(List(body=irs.uiet.body))
        cb = self._make_petsc_callable(
    ...
    def _create_initial_guess_body(self, body):
        ...
        body = self._make_callable_body(
            ...
            stacks=(vec_get_array, dm_get_local_info) + derefs
        )
        subs = {i._C_symbol: FieldFromPointer(i._C_symbol, ctx) for
                i in fields if not isinstance(i.function, AbstractFunction)}
...
class ConstrainedBCCallbackMixin:
    """
    Mixin that adds the `CountBCs` and `SetPointBCs` callbacks for solvers
    with constrained essential boundary conditions applied via PetscSection.
    This means that the constrained nodes are removed from the global solver - only
...
class MultigridCallbackMixin:
    ...
    @property
    def multigrid_metadata(self):
        return self.solve_expr.multigrid_metadata
    ...
    def _make_core(self):
        ...
        self._make_refine()
        self._make_coarsen()
        # TODO: move this stuff to a different location
        self.solver_objs['sctxc'] = ShellStruct(
            name='sctxc', pname='ShellCtx', fields=[], modifier=' *', liveness='lazy'
        )
        self.solver_objs['sctxf'] = ShellStruct(
            name='sctxf', pname='ShellCtx', fields=[], modifier=' *', liveness='lazy'
        )
        userctx = self.solver_objs['userctx']
        uctx_c = self.solver_objs['uctx_c'] = UserCtxArray(
            name='uctx_c', pname=userctx.pname, fields=[], modifier='*'
        )
        uctx_f = self.solver_objs['uctx_f'] = UserCtxArray(
            name='uctx_f', pname=userctx.pname, fields=[], modifier='*'
        )
    ...
    def _make_interpolation(self):
        ...
        cb = self._make_petsc_callable(
            ...
            parameters=(shellc, shellf, mat, vec)
        )
...
def make_callback_builder_cls(is_coupled, is_multigrid, is_initial_guess,
    ...
    return type('CallbackBuilder', tuple(mixins), {})
