...
from devito.types.dimension import CustomBoundSubDimension, SpaceDimension, CustomBoundSpaceDimension, CustomDimension
...
def constrain_essential_bcs(expressions, **kwargs):
    ...
    for e in expressions:
        ...
        for d in subdims:
            ...
            # from IPython import embed; embed()
            # unique_name 
            # TODO: should this just be CustomDimension?
            new_dim = CustomBoundSubDimension(
            name=f'{d.name}_new',
            parent=d.parent,
            thickness=d.thickness,
        ...
        for d in space_dims:
            ...
            name=sregistry.make_name(prefix=d.name + '_zoe'),
            symbolic_min=Max(space_dim_min, d.symbolic_min - halo_size_left),
            symbolic_max=Min(space_dim_max, d.symbolic_max + halo_size_right)
            )
            mapper[d] = new_dim
        #     # from IPython import embed; embed() 
    ...
    for e in expressions:
        ...
        local_mapper = {k: v for k, v in mapper.items() if k in relevant_dims}

        new_e = e.subs(local_mapper)
        if e.implicit_dims:
            implicit_dims_new = tuple(local_mapper.get(d, d) for d in e.implicit_dims)
            new_e = new_e._rebuild(implicit_dims=implicit_dims_new)
        new_exprs.append(new_e)
