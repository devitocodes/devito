"""
Structured redistribution: ``self[idx] = other`` between two distributed arrays.

Unlike the scattered engine (which routes an explicit list of points), this
handles the *structured* quadrant -- a block of ``other`` mapped into a strided
region of ``self`` -- as a point-to-point push, replacing the legacy
all-to-all broadcast. The source-to-destination mapping is derived directly from
the index as a per-axis affine map (``self_coord = start + c*step``), so it never
relies on the legacy ``_process_args`` distributor rebuilds.

It reuses the engine's :class:`Layout` and owner-resolution/transport. Patterns
it does not yet cover return ``False`` from :func:`redistribute_set`, so the
caller falls back to the legacy path -- no behavior is lost.
"""
