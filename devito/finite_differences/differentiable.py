...
class SparseLocalSum(IndexSum, Pickable):
    ...
    `cdims` are the interpolation's guarded ConditionalDimensions, retained
    with their original parents and conditions. `dimensions` exposes the parent
    iteration dimensions. Masked taps contribute zero. The sum remains symbolic
    until Cluster lowering chooses its implementation.
    ...
    def __new__(cls, expr, cdims, dtype, **kwargs):
