    ...
    def __init__(self, exprs, rules=None):
        ...
        for i, e in enumerate(exprs):
            if e.find(Barrier) | e.find(Jump):
                self.writes.setdefault(Delta, []).append(
                    TimedAccess(Delta, 'W', i, e.ispace)
                )
                self.reads.setdefault(Delta, []).extend([
                    TimedAccess(Delta, 'R', max(i, 0), e.ispace),
                    TimedAccess(Delta, 'R', i+1, e.ispace),
                ])
