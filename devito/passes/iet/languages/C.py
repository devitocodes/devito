...
class PetscCDataManager(CDataManager):
    ...
    def rebuild_callback_struct(self, iet, mapper, **kwargs):
        ...
        # from IPython import embed; embed()
        # any new fields at this point should be dereferenced at the top of efunc
        # since the `allocs` and `casts` etc will most likely depend on them.
        new_fields = []
        for f in fields:
        ...
        # Dereference the new fields and place as `standalones` at the top of the body
        # since the new symbols generated by `place_definitions` and `place_casts` likely
