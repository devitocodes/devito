...
class Test_API:
    ...
    def test_solve(self, time_int):
        ...
        print(pdes)
        for i in pdes:
            print(i,isinstance(i, MultiStage))
...
class Test_HORK_Opt:
    ...
    def test_coupled_op_computing_exp(self, time_int='HORK_EXP_OPT'):
        # Grid setup
        grid, x, y, dt, t, dx = grid_parameters(
            extent=(1, 1), shape=(201, 201))
        ...
        # Time integration scheme
        pdes = multistage_method(
            u_multi_stage, system_eqs_rhs, time_int, degree=4, source=src, optimized_feature='conv')
        op = Operator(pdes, subs=grid.spacing_map)
        op(dt=0.001, time=2000)
    ...
    def test_HORK_EXP_convergence(self, degree):
        ...
        # Time integration scheme
        pdes = multistage_method(
            u_multi_stage, eq_rhs, 'HORK_EXP_OPT', source=src, degree=degree, optimized_feature='conv')
        op = Operator(pdes, subs=grid.spacing_map)
        op(dt=dt0, time=nt)
