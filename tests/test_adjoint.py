...
class TestAdjoint:
    ...
    def test_adjoint_F(self, mkey, shape, kernel, space_order, time_order, setup_func):
        ...
        term2 = norm(rec) ** 2
        info(f'<x, A^Ty>: {term1:f}, <Ax,y>: {term2:f}, difference: {(term1 - term2)/term1:4.4e}, ratio: {term1 / term2:f}')
    ...
    def test_adjoint_J(self, mkey, shape, kernel, space_order, time_order, setup_func):
        ...
        term2 = norm(du)**2
        info(f'<x, J^Ty>: {term1:f}, <Jx,y>: {term2:f}, difference: {(term1 - term2)/term1:4.4e}, ratio: {term1 / term2:f}')
