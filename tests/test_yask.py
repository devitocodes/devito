...
from devito import (Eq, Grid, Operator, Constant, Function, TimeFunction,
                    SparseTimeFunction, configuration, clear_cache, SubDimension)  # noqa
...
class TestOperatorSimple(object):
    ...
    def test_subdims_parallel(self):
        """
    ...
    def test_subdimmiddle_notparallel(self):
        ...
        Different from case above because the interior cannot be evaluated
        in parallel.  This should prevent yaskification.
        """
        grid = Grid(shape=(20, 20))
        x, y = grid.dimensions
        t = grid.stepping_dim
    ...
    def test_subdimleft_notparallel(self):
        ...
        print(u.data[1, :, 10])
        assert np.all(u.data[1, 0:4, 10] == [1, 2, 3, 4])
        assert np.all(u.data[1, 5:, 10] == 0)
        assert np.all(u.data[1, :, 0:10] == 0)
        assert np.all(u.data[1, :, 11:] == 0)
    ...
    def test_subdimleft_parallel(self):
        ...
        assert np.all(u.data[1, 0:4, 0:thickness] == 0)
        assert np.all(u.data[1, 0:4, -thickness:] == 0)
        assert np.all(u.data[1, 0:4, thickness:-thickness] == 1)
        assert np.all(u.data[1, 5:, :] == 0)
