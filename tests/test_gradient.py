...
def test_gradientJ(dimensions, time_order, space_order):
    ...
    if len(dimensions) == 2:
        ...
        # True velocity
        true_vp = np.ones(dimensions) + .5
        true_vp[:, int(dimensions[1] / 2):] = 2.8
        ...
        # True velocity
        true_vp = np.ones(dimensions) + .5
        true_vp[:, :, int(dimensions[2] / 2):] = 2.8
    ...
    src = PointSource(name='src', data=time_series, coordinates=location)
    rec = Receiver(name='rec', ntime=nt, coordinates=receiver_coords)
    linrec = Receiver(name='rec', ntime=nt, coordinates=receiver_coords)
    wave = AcousticWaveSolver(model, source=src, receiver=rec,
                              time_order=time_order, space_order=space_order)

    # Compute receiver data and full wavefield for the smooth velocity
    rec, u0, _ = wave.forward(m=model0.m, save=False)

    # Gradient: J dm
    Jdm, _ , _, _ = wave.born(np.float32(model0.pad(dm)),
                              rec=linrec, m=model0.m)
    # FWI Gradient test
    H = [0.5, 0.25, .125, 0.0625, 0.0312, 0.015625, 0.0078125]
    error1 = np.zeros(7)
    error2 = np.zeros(7)
