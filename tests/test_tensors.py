...
def test_transpose_vs_T(func1):
    ...
    for f4i, f2i in zip(f4, f2, strict=False):
        assert f4i == f2i.T
...
def test_tensor_fd(func1):
    grid = Grid(tuple([5]*3))
    f1 = func1(name="f1", grid=grid)
    assert np.all([f.dx == f2 for f, f2 in zip(f1, f1.dx, strict=False)])
...
def test_sympy_matrix(func1):
    ...
    assert all(sp - dp == 0 for sp, dp in zip(mat * f1, mat * sympy_f1, strict=False))
    assert all(sp - dp == 0 for sp, dp in zip(f1 * vec, sympy_f1 * vec, strict=False))
...
def test_shifted_div_of_tensor(shift, ndim):
    ...
    for order in [None, 2]:
        ...
        ref = []
        for i, _a in enumerate(grid.dimensions):
            elems = []
...
def test_shifted_lap_of_vector(shift, ndim):
    ...
    for order in [None, 2]:
        ...
        for (vi, dfvi) in zip(v, df, strict=False):
...
def test_basic_arithmetic():
    ...
    # Scalar operations
    t1 = tau + 1
    assert all(t1i == ti + 1 for (t1i, ti) in zip(t1, tau, strict=False))

    t1 = tau * 2
    assert all(t1i == ti * 2 for (t1i, ti) in zip(t1, tau, strict=False))
...
def test_custom_coeffs_tensor():
    ...
    for i, _d in enumerate(grid.dimensions):
...
def test_custom_coeffs_tensor_basic(func):
    ...
    df = f.dx(w=c)
    for (fi, dfi) in zip(f.values(), df.values(), strict=False):
...
def test_rebuild(func1):
    ...
    for (i, j) in zip(f1.flat(), f2.flat(), strict=False):
    ...
    for (i, j) in zip(f1.flat(), f3.flat(), strict=False):
