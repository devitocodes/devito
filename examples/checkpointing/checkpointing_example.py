...
class CheckpointingExample(GradientExample):
    ...
    def forward_field(self):
        return TimeFunction(name="u", grid=self.model.grid, time_order=self.time_order, space_order=self.space_order, save=False)
    ...
    def gradient_operator(self):
        return GradientOperator(self.model, self.src, self.rec_g, time_order=self.time_order,
                              spc_order=self.space_order, save=False)

    def gradient(self, m0, maxmem=None):
        cp = DevitoCheckpoint([self.forward_field])
        n_checkpoints = None
        if maxmem is not None:
            n_checkpoints = int(floor(maxmem*10**6/(cp.size*self.forward_field.data.itemsize)))

        wrap_fw = CheckpointOperator(self.forward_operator, {'u': self.forward_field, 'rec': self.rec, 'm': m0, 'src': self.src, 'dt': self.dt})
        wrap_rev = CheckpointOperator(self.gradient_operator, {'u': self.forward_field, 'v': self.adjoint_field, 'm': m0, 'rec': self.rec_g, 'grad': self.grad, 'dt': self.dt})
