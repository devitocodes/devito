...
using snapshots of the wavefields.

Memory-profiler run as: mprof run Gradient_snap_marmousi.py; mprof plot
'''
import numpy as np
...
def fwi_gradient(vp_in):    
    ...
    for i in range(nshots):
        ...
        _, u0, _ = solver.forward(vp=vp_in, save=False, rec=d_syn, factor=5)
        
        # Compute gradient from data residual and update objective function 
        compute_residual(residual, d_obs, d_syn)
        
        objective += .5*norm(residual)**2
        solver.gradient(rec=residual, u=u0, vp=vp_in, grad=grad, factor=5)
...
print(f"Memory usage at the end of gradient with snapshots: {mem_grad} MiB")
