...
from examples.seismic.tti.operators import ForwardOperator, AdjointOperator, particle_velocity_fields
...
class AnisotropicWaveSolver(object):
    ...
    @memoized_meth
    def op_adj(self, kernel='centered', save=False):
    ...
    def adjoint(self, rec, srca=None, u=None, v=None, vp=None,
                epsilon=None, delta=None, theta=None, phi=None,
                save=False, kernel='centered', **kwargs):
        """
        Forward modelling function that creates the necessary
        ...
        kwargs.update(self.model.physical_params(vp=vp, epsilon=epsilon, delta=delta,
                                                 theta=theta, phi=phi))
        # Execute operator and return wavefield and receiver data
        op = self.op_adj(kernel, save)
        summary = op.apply(srca=srca, rec=rec, u=u, v=v,
                           dt=kwargs.pop('dt', self.dt), **kwargs)
        return srca, u, v, summary
