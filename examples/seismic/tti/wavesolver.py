...
from examples.seismic.tti.operators import ForwardOperator, ForwardOperator_legacy
...
class AnisotropicWaveSolver(object):
    ...
    @cached_property
    def op_fwd_legacy(self):
        """Cached operator for forward runs with buffered wavefield"""
        return ForwardOperator_legacy(self.model, save=False, source=self.source,
                                      receiver=self.receiver, time_order=self.time_order,
                                      space_order=self.space_order, **self._kwargs)

    @cached_property
    def op_fwd_save_legacy(self):
        """Cached operator for forward runs with unrolled wavefield"""
        return ForwardOperator_legacy(self.model, save=True, source=self.source,
                                      receiver=self.receiver, time_order=self.time_order,
                                      space_order=self.space_order, **self._kwargs)
    ...
    def forward_legacy(self, src=None, rec=None, u=None, v=None, m=None,
                       epsilon=None, delta=None, theta=None, phi=None,
                       save=False, **kwargs):
        """
        Forward modelling function that creates the necessary
        ...
        # Execute operator and return wavefield and receiver data
        if save:
            op = self.op_fwd_save_legacy
        else:
            op = self.op_fwd_legacy
