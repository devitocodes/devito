            ...
            for (int x = x0_blk0; x <= x0_blk0 + x0_blk0_size - 1; x += 1)
            {
              for (int y = y0_blk0; y <= y0_blk0 + y0_blk0_size - 1; y += 1)
              {
                for (int z = z0_blk0; z <= z0_blk0 + z0_blk0_size - 1; z += 1)
                {
                  ...
                  const float u31 = -7.93650813e-6F*u6 + 1.12874782e-4F*u13 - 8.8888891e-4F*u20 + 7.11111128e-3F*u27 - 3.79629639e-2F*u[t0][x + 8][y + 8][z + 8];

                  u[t2][x + 8][y + 8][z + 8] = r49[x + 8][y + 8][z + 8]*r14*(u31 + (-(u30)*r15)*r50[x + 8][y + 8][z + 8]);
                }
              }
            }
          }
