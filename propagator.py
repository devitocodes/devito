...
class Propagator:
    ...
    def prepare_loop(self, loop_body, looper_matrix_name):
        ...
        for dim_ind in range(1, num_spac_dim+1):
            ...
            dim_size = looper_matrix_name+"_size"+str(dim_ind)
            loop_limits = self._loop_limits[self.space_dims[dim_ind-1]]
            loop_body = cgen.For(cgen.InlineInitializer(cgen.Value("int", dim_var), str(loop_limits[0])),dim_var + "<" + str(loop_limits[1]), dim_var + "++",loop_body)
