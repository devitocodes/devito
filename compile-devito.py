...
cpu_pipeline = '"builtin.module(                      canonicalize, loop-invariant-code-motion, convert-scf-to-openmp,  convert-scf-to-cf, convert-openmp-to-llvm, convert-cf-to-llvm{index-bitwidth=64}, convert-math-to-llvm, convert-arith-to-llvm{index-bitwidth=64}, convert-memref-to-llvm{index-bitwidth=64}, convert-func-to-llvm, reconcile-unrealized-casts, canonicalize)"'
...
kernel_object_cmd = f'xdsl-opt {kernel_filename} -t mlir -p {xdsl_pipeline} | ' f'mlir-opt --pass-pipeline={pipeline} | mlir-translate --mlir-to-llvmir | ' f'clang-13 -x ir -c -o {kernel_object} -'
print(kernel_object_cmd)
check_call(kernel_object_cmd, shell=True)

print("Linking executable:")
link_cmd = f'clang-13 {main_object} {kernel_object} {c_object} {"-lmlir_cuda_runtime" if gpu else ""} -o {executable_name}'
print(link_cmd)
