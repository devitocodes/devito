...
if len(sys.argv) < 2 + dims[benchmark] or len(sys.argv) > 4 + dims[benchmark]:
    print(f"usage: {sys.argv[0]} <benchmark> <size, e.g 2 2 for 2d>[initial threads] [maximum threads]")
...
init_threads = int(sys.argv[2+dims[benchmark]]) if len(sys.argv) >= 3+dims[benchmark] else 1
max_threads = int(sys.argv[3+dims[benchmark]]) if len(sys.argv) >= 4+dims[benchmark] else mp.cpu_count()
...
def get_runtimes_for_threads(threads : int) -> tuple[int, float, float]:
    ...
    try:
        ...
        pair = (threads, float(xdsl_line.split(" ")[-2]), float(devito_line.split(" ")[-2]))
